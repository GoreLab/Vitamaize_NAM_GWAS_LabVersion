...
placeholder.file.location = "/Expression_Analyses/GMMS3.1Results_tocos/Placeholders/"
...
### Added by CHD 7/15: File size 311 checks for files that just have header line--in this case, will pipe that QTL/trait combination through 'else'
 if(file.exists(file.name) & file.info(file.name)$size > 311){
   ...
   print(paste("No imputed SNP file exists for trait ",trait," common SI ",common.SI," chr ",chr,". Not even header was printed. Please check!",sep = ""))
    #setwd(paste(home.dir,placeholder.file.location,sep = "")) 
   #GWAS.SNPs.on.Chr <- GAPIT.Fragment(file.G=paste("mock.placeholder.hapmap.snp.file.chr",sep = ""),                       #######################
   #                    file.Ext.G= "txt", file = chr, frag = 1, genoFormat="hapmap", file.fragment=300) 
   #setwd(paste(home.dir,get.me.my.SNPs.files.dir,imputed.file.location,sep = ""))    #commented out by CHD 7/15/2015--not needed
...
for(i in list.of.gene.GRZMs.in.support.interval){
  ...
  for(j in 1:ncol(the.FPKM.values)){
    ...
    for(k in 1:ncol(numeric.GWAS.SNPs.data.temp)){
        ...
        Correl.Pearson <- tryCatch({
          cor(as.numeric(as.character(SNP.alleles.and.FPKM[,2])), as.numeric(as.character(SNP.alleles.and.FPKM[,3])), 
          ...
          print(paste(c("stdev for imputed was 0: allele vector ",these.alleles,"FPKM vector ",these.FPKMs),collapse=','))
        }
        )
...
for(i in list.of.gene.GRZMs.in.support.interval){
  ...
  for(j in 1:ncol(the.FPKM.values)){
    ...
    Correl.Pearson <- tryCatch({
      cor(as.numeric(as.character(JL.effects.and.this.FPKM[,3])), as.numeric(as.character(JL.effects.and.this.FPKM[,4])), 
                          use = "pairwise.complete.obs", method = "pearson") 
    }, warning = function(w) {
      these.effects = as.vector(JL.effects.and.this.FPKM[,3])
      these.FPKMs = as.vector(JL.effects.and.this.FPKM[,4])
      print(paste(common.SI,trait,i,sep=" "))
      print(paste(c("stdev for ordered was 0: effect vector ",these.effects,"FPKM vector ", these.FPKMs),collapse=','))
    }
    )
