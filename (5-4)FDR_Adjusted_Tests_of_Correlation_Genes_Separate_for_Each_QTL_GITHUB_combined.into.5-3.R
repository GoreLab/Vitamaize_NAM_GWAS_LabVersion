...
get.me.my.FPKM.values <- function(absolute.final.data.set.FPKM = NA, gene.ID = NA, gene.name = NA,
  ...
  if(print.out.results){
    setwd(paste(home.dir, FPKM.output.byQTL.dir, sep = ""))
...
correlate.it.dude <- function(bt.effect.estimates.from.QTL =NULL, tabular.summary = tabular.summary, Gene.GRZM.ID = NA,  absolute.final.data.set.FPKM = absolute.final.data.set.FPKM, trait = trait, QTL=QTL){
  ...
  for(i in list.of.gene.GRZMs.in.support.interval){
    ...
    the.FPKM.values <- get.me.my.FPKM.values(absolute.final.data.set.FPKM = absolute.final.data.set.FPKM, 
                                             ...
                                             print.out.results = FALSE, output.dir = FPKM.output.byQTL.dir, home.dir = home.dir)
...
if(trait.set == "Carot"){
  ...
  FPKM.output.byQTL.dir <- "\\(15)Correlated Expression\\JL_GWAS_FPKM_overlap_analysis_2015\\Significance Threshold\\FDR_Test_Ancillary_Output_Files\\"
...
if(trait.set == "tocos"){
  ...
  get.me.my.SNPs.files.dir <-"/Expression_Analyses/GMMS3.1Results_tocos/ImputedMatricesfrom5.1/" #CHD formerly named dir.for.5.1.matrices
  ...
  FPKM.output.byQTL.dir = "/Expression_Analyses/GMMS3.1Results_tocos/FPKMmatricesFrom5.4/"
