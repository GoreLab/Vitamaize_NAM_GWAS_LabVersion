...
bash null_dist.sh -t aT3 -r ../../data_input/Resid.totalTocochrs.SI.01_2015.txt -p 1 -i ../../data_input/ -o ../analysis_output/Complete_Results_BashPart1/ -a 13 -s 10 -c 1 -V -k #run on CBSU server Sun to Mon 4/12-13/2015 CHD, restarted on chr 10 using same perm files (edit made by Dan 4/13). 4/22 re-ran chr 10 on lab server so that results (model output) would not append, rather only 1 line per file.
sleep 2m
#cp -r -n analysis_output /home/mag87_0001/chd45/
#ulimit -Su $(ulimit -Hu)
