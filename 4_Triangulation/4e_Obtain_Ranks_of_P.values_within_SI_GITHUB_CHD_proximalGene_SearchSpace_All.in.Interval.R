...
if(user == 'chd'){
  ...
  #dir.for.compil.of.tri.summ = "/Expression_Analyses/GMMS3.1Results_tocos/orderedFrom5.2/" #this was pre-dT3 redo
  dir.for.compil.of.tri.summ = "C:/Users/chd45/Documents/Data_From_Mac/Documents/Toco_NAM/Expression/orderedFrom5.2/"
  trait.set = "tocos" #Options are "Carot" or "tocos"
  tabSummary.path = "C:/Users/chd45/Documents/Data_From_Mac/Documents/Toco_NAM/Summary_Tables.Figures/"
  location.of.GWAS.results = "/GWAS_Analysis/GWAS_25fam_HMPonly_TASSEL3_alpha01_2015_corr/"
  
  #Read in the tabular summary; used below to obtain a list of trait names
  tabular.summary <- read.table(paste(tabSummary.path,"Tab_Sum_",trait.set,"_alpha.01_SI_with_Overlapping_GWAS_SNPs_2015_dT3_Redone.txt", sep = ""), head = TRUE)
}
...
#RMIP.commonSI.key = read.table(paste(home.dir,tabSummary.path,"Complete_Results_allTraits_RMIPgt4_with_QTLnumber_dupSNPremoved.txt", sep = ""), head = TRUE) #This was pre-dT3 redo.
RMIP.commonSI.key = read.table(paste(tabSummary.path,"Complete_Results_allTraits_RMIPgt4_dT3.Redone_with_QTLnumber.txt", sep = ""), head = TRUE)

#For loop through each common S.I.(39 for carot, 48 for toco)
#for(cSI in unique(tabular.summary[,12])){ 
  for(cSI in c(1,2,3,4,5,8,14,18,21,23,24,26,29,30,31,33,35,36,37,38,41,42,43,45,48,49,52)){#these were affected by dT3 redo
  ...
  for(trait in trait.list){
    ...
    setwd(paste(dir.for.compil.of.tri.summ,"QTL_",cSI,"_imputed.ordered.tri.files/", sep=''))
    ...
    #Extract RMIP common S.I. key rows corresponding to this trait-common S.I. combinationRMIP.SNPs.this.trait_
    RMIP.SNPs.this.trait_cSI = RMIP.commonSI.key[which(RMIP.commonSI.key[,1]==trait & RMIP.commonSI.key[,6]==cSI),] #note trait was formerly in column 5 & this object was formerly named a nondescriptive "cSI"
  ...
  if(pvals.to.output == 'all.in.interval'){
    ...
    write.table(FDR.pval.this.cSI_allTraits,paste("FDR.P.values_all.in.interval_QTL",cSI,"_dT3.redone.txt",sep=''),sep='\t',row.names=FALSE)
  }# end all.in.interval
  if(pvals.to.output == "search.range"){
    ...
    write.table(fits.criteria.raw.master, paste("Raw.P_Genes.", search.range,"bp.of.GWAS.SNPs_QTL",cSI,"_dT3.redone.txt", sep = ""), sep = "\t", quote = FALSE,row.names=FALSE)
    #FDR.search.range.sorted = fits.criteria.FDR.master[order("trait","Chr","Start_bp"),]
    write.table(fits.criteria.FDR.master, paste("FDR.P_Genes.", search.range,"bp.of.GWAS.SNPs_QTL",cSI,"_dT3.redone.txt", sep = ""), sep = "\t", quote = FALSE)  
    }#end if is not null
    }# end search.range
