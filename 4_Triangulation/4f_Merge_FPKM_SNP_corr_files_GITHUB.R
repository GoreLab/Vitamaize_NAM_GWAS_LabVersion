...
for(cSI in unique(tabular.summary[,column.of.cSI.in.tab.sum])){
  print(paste("For common support interval number ",cSI,":",sep=''))
    ...
    #obtain all FPKM.SNP correlation files from specified folder
     ...
     FPKM.SNP.files <- file.list[grep("IMPUTED", file.list)]
     print(FPKM.SNP.files)
    
    initialize.data.frame <- NULL
       
    #read in FPKM, SNP files, one at a time
    for(pick.one in 1:length(FPKM.SNP.files)){
            data.file <- read.table(FPKM.SNP.files[pick.one], head = TRUE, stringsAsFactors=F)
            ...
            ######This should work
            pattern <- substr(FPKM.SNP.files[pick.one], 30, 33)
          ...
          #some housekeeping
            ...
            #add trait name to each pos_SNP column name - special case where there is only one SNP is listed first
            #if(length(ncol(data.file)) == 2){
            if(ncol(data.file) == 2){
                colname.vector <- colnames(data.file)[2]
                new.colname.vector <- paste(trait.name, "_", colname.vector, sep = "")
                colnames(data.file)[2] <- new.colname.vector
            }else{
                colname.vector <- colnames(data.file)[2:ncol(data.file)]
                new.colname.vector <- paste(trait.name, "_", colname.vector, sep = "")
                colnames(data.file)[2:ncol(data.file)] <- new.colname.vector
            } #end adding trait names
            ...
            positional.data <- absolute.final.data.set.FPKM[absolute.final.data.set.FPKM[,1] %in% gene.vector.remove.suffix, c(1:2,4:5)]
            ...
            #add positional data to data.file, again, special case where there is only 1 SNP
            if(ncol(data.file) == 2){
                new.data.file <- cbind(data.file[,1], pos.for.file, data.file[,2] , stringsAsFactors=F)
                colnames(new.data.file)[1] <- colnames(data.file)[1]
                colnames(new.data.file)[6] <- colnames(data.file)[2]
            }else{
                new.data.file <- cbind(data.file[,1], pos.for.file, data.file[,2:ncol(data.file)] , stringsAsFactors=F)
                colnames(new.data.file)[1] <- colnames(data.file)[1]
            } #end adding positional data

            #ensure data file is sorted by gene start position
            #new.data.file <- data.file
            new.data.file <- new.data.file[order(new.data.file[,4]),]

      if((pick.one == 1) && (length(FPKM.SNP.files) == 1)) {                 
            ...
            if(pick.one == 1){
              ...
              new.data.file -> query.data.frame#}       opened loop to include code up to line 168
                   ...
                   min.initial <- min(initialize.data.frame[,4], na.rm = TRUE)  #ADDRESS no NAs allowed to be considered in min
                   min.query <-  min(query.data.frame[,4])
                   ...
                   #SNPs.in.start.list <- ncol(start.list)-4 # 4 columns with identity information
                   #SNPs.in.query.list <- ncol(query.list)-4 # 4 columns with identity information
                  ...
                  for (n in 1:nrow(query.list)){
                   ...
                   if(length(nrow(non.matched.rows)) == 0){break}else{ 
                      ...
                      start.list.mod[start.NA.row:(start.NA.row+rows.needed-1), 1:5] <- non.matched.rows[,1:5]
     ...
     #more housekeeping
        ...
        #start.list.mod <- start.list.mod[,-c((ncol(start.list.mod)-SNPs.in.query.list-3):(ncol(start.list.mod)-SNPs.in.query.list))]
        ...
        #remove columns with query row identity
            ...
            for(header.ID in c("Gene_in_SI", "gene_locus", "description", "left", "right")){
                remove.these.columns <- grep(header.ID, colnames(initialize.data.frame[6:ncol(initialize.data.frame)]))
            ...
            if(length(all.columns.to.be.removed) > 0){
                ...
                all.columns.to.be.removed.mod <- all.columns.to.be.removed + 5
        ...
        initialize.data.frame[,6:ncol(initialize.data.frame)] = round(initialize.data.frame[,6:ncol(initialize.data.frame)],3)
        
        #change all NAs to "." to facilitate reading matrix in excel
        initialize.data.frame[is.na(initialize.data.frame)] = "."
        
        #output table
        write.table(initialize.data.frame, paste("All.trait.FPKM.by.SNP.corr.summary.file.for.cSI.",cSI,".txt", sep = ""),  sep = "\t", quote = FALSE, row.names = FALSE)
