...
pleiotropy.calculator.within.supp.int <- function(trait.1, trait.2, threshold, validation = FALSE){
  ...
  print(paste("Threshold is ", threshold, sep=''))
      ...
      #Read in the results for Trait 1.
        ...
        if(trait.1 == "dT3"){
          Results.trait.1 <- read.table(paste(dT3.new.path,"new.trans_new.perm_FINAL/Pop.by.Marker.Effect.Estimates.from.R.dT3_Redone.SI01.txt",sep=''),head=TRUE)
        }else{
      ...
      pheno.data = read.table(paste(home.dir,pheno.dir,"BLUEs_No_Outliers_Transformed_all_for_TASSEL_",trait.2,".txt", sep = ""), head = TRUE)
      }
      ...
      for(i in unique(marker.terms)){
       ...
       if( (length(cor.results.signif)>0) ){ 
        ...
        write.table(cor.results.signif, paste(home.dir,pleio.dir,"/Signif_Pleio_Pairwise_Lists/",trait.1,".QTL",
                                              "that.are.SIGNIF.pleiotropic.with",trait.2,"_0.01_sign.corrected.txt", sep = ""), row.names = FALSE, sep = "\t",  quote = FALSE)
...
pop.X.mkr.dir = "/Summary_Tables.Figures/for_EffEst_Figs/Allelic_Effect_Estimates.no.MultiColl/"
pheno.dir = "/Geno.Pheno_Inputs/"
geno.dir = "/Geno.Pheno_Inputs/"
dT3.new.path = "C:/Users/chd45/Documents/Data_From_Mac/Documents/Toco_NAM/Methods/dT3_removeExtremeVal_test/"
...
JL.Results <- read.table(paste(home.dir,tabSummary.path,"Tab_Sum_tocos_alpha.01_SI_with_Overlapping_GWAS_SNPs_2015_dT3_Redone.txt",sep=''), head = TRUE)
...
#Estimate Pleiotropy
traits <- c("aT","aT3","dT","dT3","gT","gT3","PC8","totalT","totalT3","totalTocochrs")
