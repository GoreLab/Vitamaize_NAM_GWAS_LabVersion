...
trait.set = "tocos" #Options: "Carot" or "tocos"
threshold=0.01
...
tab.summary <- read.table(paste(home.dir,tabSummary.path,"Tab_Sum_tocos_alpha.01_SI_with_Overlapping_GWAS_SNPs_2015_dT3_Redone.txt",sep=''), head = TRUE)
...
for(i in 1:nrow(tab.summary)){ #for loop through i (1 to nrows(tab summary))
  for(j in as.character(unique(tab.summary[,1]))){ #for loop through j in 1:length(unique(tab.summary[,1])))
    ...
    if(trait.a == "Total_Tocopherols"){trait.a = "totalT"}

    #trait.b will be jth row 1st column
    trait.b <- j
    if(trait.b == "Total_Tocopherols"){trait.b = "totalT"}
    ...
    QTL.test.table <- try(read.table(paste(home.dir, pleio.data.dir,trait.a,".QTLthat.are.SIGNIF.pleiotropic.with",trait.b,"_",threshold,"_sign.corrected.txt",sep = ""),stringsAsFactors=FALSE))
...
write.table(pleio.output.matrix, paste(home.dir,pleio.matrix.dir,"Pleiotropic.Output.Matrix.for.",trait.set,".SI01_with.dT3.redone_sign.corrected.txt", sep = ""), 
...
write.table(merge.file, paste(home.dir,pleio.matrix.dir,"Pleiotropic.Output.Matrix.for.",trait.set,".SI01.TAB.SUM.MERGE_with.dT3.redone_sign.corrected.txt", sep = ""), 
